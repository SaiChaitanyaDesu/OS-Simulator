...
// Clear a partition when process is removed
function clearPartition(index) {
  var ctx = document.getElementById("myCanvas").getContext("2d");
  
  // Clear the partition
  ctx.fillStyle = "#ffffff";
  ctx.fillRect(xstart, partitionStart[index], canvasWidth, partitionEnd[index] - partitionStart[index]);
  
  // Redraw partition border
  ctx.strokeStyle = "#000000";
  ctx.strokeRect(xstart, partitionStart[index], canvasWidth, partitionEnd[index] - partitionStart[index]);
  
  // Add partition number
  ctx.fillStyle = "#000000";
  ctx.font = "12px Arial";
  ctx.fillText("P" + (index + 1) + ": " + partition_size[index] + "KB", xstart + 5, partitionStart[index] + 15);
}
...
function drawMemory() {
  ...
  for (var i = 0; i < noOfPartitions; i++) {
    partition_size[i] = Math.floor(Math.random() * (memsize / 2)) + (memsize / 4);
    totalSize += partition_size[i];
  }
  
  // Scale factor to normalize partition sizes to fit canvas
  var scaleFactor = canvasHeight / totalSize;
  ...
  for (var i = 0; i < noOfPartitions; i++) {
    ...
    // Calculate partition height proportional to its size
    var partHeight = partition_size[i] * scaleFactor;
    
    partitionStart[i] = currentY;
    partitionEnd[i] = currentY + partHeight;
    
    // Draw partition
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(xstart, partitionStart[i], canvasWidth, partHeight);
    
    // Draw partition border
    ctx.strokeStyle = "#000000";
    ctx.strokeRect(xstart, partitionStart[i], canvasWidth, partHeight);
    
    // Add partition number and size
    ctx.fillStyle = "#000000";
    ctx.font = "12px Arial";
    ctx.fillText("P" + (i + 1) + ": " + partition_size[i] + "KB", xstart + 5, partitionStart[i] + 15);
    
    currentY += partHeight;
  }
  
  total_mem_size = totalSize;
  externalFragments = memsize - totalSize;
...
function drawProcess(process_size, currProcessId, index) {
  var ctx = document.getElementById("myCanvas").getContext("2d");
  var partHeight = partitionEnd[index] - partitionStart[index];
  
  // Calculate process height proportional to partition
  var processRatio = process_size / partition_size[index];
  var processHeight = partHeight * processRatio;
  
  // Draw process
  ctx.fillStyle = "#B0DAFF";
  ctx.fillRect(xstart, partitionStart[index], canvasWidth, processHeight);
  
  // Draw internal fragmentation
  if (process_size < partition_size[index]) {
    ctx.fillStyle = "#FEFF86";
    ctx.fillRect(xstart, partitionStart[index] + processHeight, canvasWidth, partHeight - processHeight);
  }
  
  // Draw border
  ctx.strokeStyle = "#000000";
  ctx.strokeRect(xstart, partitionStart[index], canvasWidth, partHeight);
  
  // Add process ID
  ctx.fillStyle = "#000000";
  ctx.font = "14px Arial bold";
  ctx.fillText("P-" + currProcessId + " (" + process_size + "KB)", xstart + 10, partitionStart[index] + processHeight/2);
  
  // Add fragmentation info if any
  if (process_size < partition_size[index]) {
    ctx.fillStyle = "#FF55BB";
    ctx.font = "12px Arial";
    ctx.fillText("Internal: " + (partition_size[index] - process_size) + "KB", 
                 xstart + 10, partitionStart[index] + processHeight + (partHeight - processHeight)/2);
  }
}
...
function drawLegend() {
  ...
  // Empty partition
  ctx.fillStyle = "#FFFFFF";
  ctx.fillRect(10, 70, 30, 20);
  ctx.strokeRect(10, 70, 30, 20);
  
  // Labels
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "12px Arial";
  ctx.fillText("Process Used Memory", 50, 25);
  ctx.fillText("Internal Fragmentation", 50, 55);
  ctx.fillText("Empty Partition", 50, 85);
}